...
const SellingList = () => {
  ...
  const fetchSellings = async () => {
    setLoading(true);
    try {
      const url = new URL("https://bolicheck.onrender.comender.com/api/staff/sales/");
  ...
  useEffect(() => {
    fecha == "hoy"
      ? setUrl("daily_report/")
      : fecha == "semana"
      ? setUrl("weekly_report/")
      : fecha == "mes"
      ? setUrl("monthly_report/")
      : fecha == "año"
      ? setUrl("yearly_report/")
      : fecha == "todas"
      ? setUrl("total_report/")
      : "";
    fetchSellings();
  ...
  const handleConfirm = async (id) => {
    try {
      const response = await fetchWithAuth(
        `https://bolicheck.onrender.comender.com/api/staff/sales/${id}/confirm/`,
        {
          method: "POST",
        }
      );
  ...
  const handleReport = async () => {
    ...
    try {
      ...
      if (url == "") {
        toast.error("No hay un filtro de fecha seleccionado.");
      }
      const response = await fetchWithAuth(
        `https://bolicheck.onrender.com/api/staff/sales/${url}`,
        {
          method: "POST",
        }
      );
    ...
    >
      <div className="col-12 col-lg-10 mx-auto h-auto rounded-4 form-style">
        <div className="d-flex justify-content-between">
          ...
          <h1 className="text-center col-4 mt-3">Lista de Ventas</h1>
          <Link
            to="/admin/sellings/new"
        ...
        <ul className="list-group mt-4 col-11 mx-auto mb-3 py-3">
          {loading ? (
              ...
              >
                <div className="d-flex justify-content-between align-items-center">
                  <div className="d-flex align-items-center justify-content-between col-9">
                      ...
                      <div className="col-10 col-lg-8">
                        <h5 className="mb-1">
                          ...
                          {selling.reserve.length != 0 && (
                            <span className="rounded-5 reserve-badge">
                              Reservada
                            </span>
                          )}
                          {selling.is_redemption == true && (
                            <span className="rounded-5 redemption-badge">
                              Redención
                            </span>
                          )}
                        </h5>
                        ...
                        <h5>
                          <b>
                            ...
                            {selling.is_redemption
                              ? `${selling.prizes.reduce(
                                  (acc, prize) => acc + prize.subtotal,
                                  0
                                )} pts.`
                              : `$${selling.total}`}
                          </b>
                    ...
                    <div className="align-self-center text-center col-4">
                      {selling.status === "completada" ? (
                        <>
                          <i>
                            Atendida por:{" "}
                            {selling?.by?.username ||
                              `${selling?.by?.first_name} ${
                                selling?.by?.last_name || ""
                              }`}
                          </i>
                          <br />
                          <i>
                            El:{" "}
                            {new Date(
                              selling.confirmed_at.replace(" ", "T")
                        ...
                        <div className="btn-group d-flex flex-column flex-lg-row gap-3 gap-lg-1 col-12">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                          ...
                          <button
                            ...
                            className="btn btn-sm delete-btn-style"
                          >
                            Cancelar
                          </button>
                        </div>
            ...
            <div className="text-center mt-3 mb-4">
              No hay ventas con el filtro "{searchTerm}"
              {estado != "todos" ? ` con estado ${estado}` : ""}
              {fecha === "hoy" && " registradas hoy."}
              {fecha === "semana" && " registradas esta semana."}
              {fecha === "mes" && " registradas este mes."}
              {fecha === "año" && " registradas este año."}
              {!fecha && " registradas."}
            </div>
          )}
        </ul>
